...
for ((i=min_loop; i<=max_loop; i++)); do
    ...
    load_ycsb_workload "$consistency_level" "$nthreads" "$transaction_mode" "$(echo "${consistency_level}")_${i}_load_$workload.dat" "$workload" "$record_count" "$operation_count"

    echo "Running YCSB benchmark for $i node(s)..."
    run_ycsb_benchmark "$consistency_level" "$nthreads" "$(echo "${consistency_level}")_${i}_run_$workload.dat" "$workload" "$record_count" "$operation_count"
