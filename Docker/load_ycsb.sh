...
run_ycsb() {
    ...
    cmd="${debug} $ycsb_dir/bin/ycsb.sh $action cassandra-cql -p hosts=$hosts -p port=$port -P $ycsb_dir/$workload -p cassandra.writeconsistencylevel=$consistency_level -p cassandra.readconsistencylevel=$consistency_level -p recordcount=$recordcount -p operationcount=$operationcount -threads $threads -s"
...
run_ycsb "load" "$ycsb_dir" "$workload" "$hosts" "$port" "$record_count" "$operation_count" "$consistency_level" "$output_file" "$nthreads"
